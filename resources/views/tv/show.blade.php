@extends('layout.app')
@section('content')
    <div class="tv-info bg-cover bg-no-repeat" style="background-image: url(' {{ 'https://image.tmdb.org/t/p/original/' . $tvDetails['backdrop_path'] }} ')">
        <div style="background-image: linear-gradient(to right, rgba(7.84%, 8.63%, 9.80%, 1.00) 150px, rgba(7.84%, 8.63%, 9.80%, 0.84) 100%)">
            <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
                <div class="flex-none">
                    <img class="w-72" src="{{ 'https://image.tmdb.org/t/p/w500/' . $tvDetails['poster_path'] }}" alt="">
                </div>
                <div class="md:ml-24">
                    <h2 class="text-4xl font-semibold text-white">{{ $tvDetails['name'] }}</h2>
                    <div class="flex items-center text-gray-400 text-sm">
                        ...
                        <span class="ml-1">{{ $tvDetails['vote_average'] * 10 . '%' }}</span>
                        <span class="mx-2">|</span>
                        <span>{{ \Carbon\Carbon::parse($tvDetails['first_air_date'])->format('M d, y') }}</span>
                        <span class="mx-2">|</span>
                        <span>
                            @foreach ($tvDetails['genres'] as $genres)
                    ...
                    <div class="mt-4">
                        ...
                        <p class="text-gray-100">{{ $tvDetails['overview'] }}</p>
                    </div>
                    <div class="mt-12">
                        <div class="flex mt-4">
                            @foreach ($tvDetails['credits']['crew'] as $crew)
                                @if ($loop->index < 3)
                                    <div class="mr-12">
    ...
    <div class="tv-cast">
        <div class="mx-auto px-4 py-16">
            ...
            <div class="grid lg:grid-cols-7 gap-8">
                @foreach ($tvDetails['credits']['cast'] as $cast)
                    @if ($loop->index < 7)
                        <div class="mt-8">
    ...
    <div class="tv-image">
        <div class="container mx-auto px-4 py-16">
            ...
            <div class="grid lg:grid-cols-4 gap-8">
                @foreach ($tvDetails['images']['backdrops'] as $image)
                    @if ($loop->index < 8)
                        <div class="mt-8">
                        <a href="#">
                            <img src="{{ 'https://image.tmdb.org/t/p/w500/' . $image['file_path'] }}" alt="" class="hover:opacity-50 transition ease-in-out duration-150 rounded-lg">
                        </a>
                        </div>
                    @endif
                @endforeach
    ...
    <div class="tv-recommendation">
        <div class="container mx-auto px-4 py-16">
            <h2 class="capitalize text-white text-2xl font-semibold">Recommendation</h2>
            <div class="grid lg:grid-cols-7 gap-8">
                @foreach ($tvRecommendations as $tv)
                    @if ($loop->index < 7)
                        <div class="mt-8">
                            @if ($tv['poster_path'] !== null)
                                <a href="{{ route('tv.show', $tv['id']) }}">
                                    <img src="{{ 'https://image.tmdb.org/t/p/w500/' . $tv['poster_path'] }}" class="hover:opacity-50 transition ease-in-out duration-150 rounded-lg">
                                </a>
                            @else
                                <a href="{{ route('tv.show', $tv['id']) }}">
                                    <img src="http://placehold.jp/300x450.png" class="hover:opacity-50 transition ease-in-out duration-150 rounded-lg">
                                </a>
                            @endif
                            <div class="mt-2">
                                <a href="{{ route('tv.show', $tv['id']) }}" class="text-md pt-4 text-white font-semibold hover:text-yellow-500">
                                    {{ $tv['name'] }}
                                </a>
                                <div class="flex items-center text-gray-400 text-sm">
